...
impl<P: SchedulerPolicy> URXRuntime<P> {
    ...
    fn topo_order(&self, graph: &IRGraph) -> Vec<String> {
    ...
    pub async fn execute_graph(&mut self, graph: &IRGraph) -> RuntimeResult {
        ...
        let order = self.topo_order(&fused);
        ...
        for block_id in order {
            ...
            let node_id = block_binding.get(&block.block_id).unwrap().clone();
            let ctx = inbox.get(&block.block_id).unwrap();
            let value = BlockExecutor::exec(block, ctx);
            ...
            for e in fused.edges.iter().filter(|e| e.src_block == block.block_id) {
                ...
                inbox.entry(e.dst_block.clone()).or_default().insert(e.input_key.clone(), recv_value);
